...
public class AdminController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<AdminResponseDto> getAdminById(@Valid @PathVariable @Min(1) long id){
    ...
    @PutMapping("/{id}")
    public ResponseEntity<AdminResponseDto> updateAdminById(@Valid @PathVariable @Min(1) long id, @Valid @RequestBody AdminRegisterDto dto){
        return new ResponseEntity<>(service.updateAdmin(id,dto),HttpStatus.OK);
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<HttpStatus> deleteAdminById(@Valid @PathVariable @Min(1) long id){
